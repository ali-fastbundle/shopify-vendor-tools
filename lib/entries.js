...
export function sanitiseEntry(input = {}, { existingIds = [] } = {}) {
  ...
  return {
    entry: {
      ...
      verified: false,
      /*
       * Never set from here either, and for a sharper reason than `verified`.
       * `editorInterest` is a statement about the person maintaining the
       * directory, not about the tool, so nothing that arrives over HTTP or out
       * of a model gets to assert it. It is written by hand in lib/tools.js,
       * where somebody is on the hook for it in git. Absent is the only value
       * this path can produce, in either direction.
       */
