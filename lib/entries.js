...
export function sanitiseEntry(input = {}, { existingIds = [] } = {}) {
  ...
  return {
    entry: {
      ...
      verified: false,
      /*
       * `noRecommend` is never set from here either, in either direction. It is
       * a rule about this directory's own behaviour, so it is written by hand
       * in lib/tools.js where somebody is on the hook for it in git, rather
       * than asserted by anything arriving over HTTP or out of a model.
       */
