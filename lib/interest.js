 ...
 *
 * **Never throws.** `write()` rejects when the store is unreachable, and this
 * is called on a path whose whole contract is that it cannot fail: the person
 * asked a question, the answer is a link, and losing a counter is not a reason
 * to hand them an error instead. A failure is logged and 0 comes back, which
 * the caller renders as "already listed" without a number.
 */
export async function addInterest(toolId, person = {}) {
  try {
    ...
    console.error("[interest] could not record interest in", toolId, "—", e.message);
    return 0;
  }
}
...
export async function carryInterest(toolId, { count = 0, people = [] } = {}) {
  ...
  /* Same reasoning as addInterest: losing the carried count is a shame, and
     failing the publish that a person just approved is worse. */
  try {
    return await carry(toolId, wanted, people);
  } catch (e) {
    console.error("[interest] could not carry interest onto", toolId, "—", e.message);
    return 0;
  }
}

async function carry(toolId, wanted, people) {
  const all = await getInterest();
  const current = all[toolId] || { count: 0, people: [] };
