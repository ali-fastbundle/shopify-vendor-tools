 ...
 * `cat`, `verified`, ratings or reviews. That line is the point of the
 * directory, so it is enforced server-side rather than just hidden in the UI.
 */
...
export async function mergedTools() {
  const [overrides, claims] = await Promise.all([getOverrides(), getClaims()]);
  return TOOLS.map((t) => {
...
export function sanitiseEdit(input) {
  ...
  if ("social" in input && input.social) {
    const s = {};
    for (const k of ["li", "x", "gh"]) {
      if (!(k in input.social)) continue;
      const u = httpsUrl(input.social[k]);
      if (u === null) return { error: `That ${k === "li" ? "LinkedIn" : k === "x" ? "X" : "GitHub"} URL does not look right.` };
      if (u) s[k] = u;
    }
    out.social = s;
  }
...
/** Admin-only. Drops the claim entirely; the vendor's edits in svt:overrides stay. */
export async function revokeClaim(toolId) {
  const claims = await getClaims();
