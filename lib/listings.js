 ...
 *
 * `editorInterest` is here because it is the one field whose whole purpose is
 * to be inconvenient to the person it describes. An override that could clear
 * it would remove the disclosure from the entry it exists for, and one that
 * could set it would let it be planted on a competitor.
 */
export const PROTECTED = [
  "watch", "cat", "alsoIn", "verified", "ratings", "updated", "id", "name", "editorInterest",
];
...
export async function mergedTools() {
  ...
  return base.map((raw) => {
    ...
    return {
      ...
      editorInterest: t.editorInterest,
    };
  });
}
