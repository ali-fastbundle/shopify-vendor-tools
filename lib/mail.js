...
const MATRIX = {
  ...
  review: {
    admin: (d) => ({
      subject: `${d.rating}★ ${d.edited ? "revised review" : "review"} of ${d.toolName}`,
      paragraphs: [
        `Tool: ${d.toolName}`,
        `Rating: ${d.rating} out of 5`,
        `By: ${d.author || "Anonymous"} (${d.email})`,
        d.text ? `Wrote: ${d.text}` : "No text, rating only.",
        d.edited ? "This replaced their earlier review of the same tool." : "",
      ],
      button: adminButton(d.origin),
    }),
    /*
     * Reviewing requires signing in now, so there is always an address. The
     * guard stays because the matrix is the contract and a caller that omits
     * the field should send nothing rather than throw.
     */
    user: (d) => (d.email ? {
      to: d.email,
      subject: d.edited
        ? `Your review of ${d.toolName} is updated`
        : `Thanks for reviewing ${d.toolName}`,
      paragraphs: [
        d.edited
          ? `Your review of ${d.toolName} now reads ${d.rating} stars. It replaced the one you left before, rather than appearing beside it.`
        ...
        "You can change it whenever you like. One review per account per tool, so editing replaces it rather than adding another.",
      ],
  ...
  suggestion: {
    admin: (d) => ({
      subject: d.outOfScope ? `Out of scope: ${d.name}` : `New suggestion: ${d.name}`,
      paragraphs: [
        `Kind: ${d.kindLabel}`,
        ...
        d.why ? `Why: ${d.why}` : "",
        d.alsoAsked
          ? `This is a repeat. ${d.alsoAsked} people have now asked for it, and it was folded into the row already in the queue rather than filed as a new one.`
          : "",
        d.outOfScope
          ? `Out of scope: this reads as a Shopify app for merchants rather than a tool for app vendors.${d.outOfScopeReason ? ` ${d.outOfScopeReason}` : ""} It is stored and flagged, it is not in the public list, and it needs no action from you. It is in Catalogue → out of scope on /admin if the call looks wrong.`
          : "",
        d.approved === false
    ...
    user: (d) => (d.email ? {
      ...
      paragraphs: [
        ...
        d.alsoAsked
          ? `You are not the first: ${d.alsoAsked} people have asked for it now. That is recorded against the one entry rather than as separate requests, which is what makes the demand visible to us.`
          : "",
        d.why ? `You said: “${d.why}”` : "",
        /* Never tell somebody their suggestion goes in when it does not. */
        d.outOfScope
          ? "This one will not be listed, and it is worth saying why plainly. This directory covers tools for the people who build Shopify apps: rank trackers, revenue analytics, store databases, partner platforms. It does not cover Shopify apps built for merchants to run their shop, which is what this looks like. The Shopify App Store is where those belong, and it will serve you far better than we could."
          : "It goes in after a check. We read the vendor's own site before writing an entry, and anything we cannot confirm there is left out rather than published as fact. That takes a little time.",
        d.outOfScope
          ? "We have kept a note of it anyway. What people come here looking for is useful to know, and if we have the boundary wrong we would rather find out from a pile of suggestions like yours."
          : "",
  ...
  /*
   * The weekly change digest. Admin only, and only sent when something
   * actually changed: the route does not call this on a quiet week, because an
   * empty digest every Monday is how a digest becomes something people filter
   * into a folder and stop reading.
   *
   * The subject carries the count, so the inbox line alone says whether this
   * is a week that needs half an hour.
   */
  monitor_digest: {
    admin: (d) => ({
      subject: `${d.count} recent update${d.count === 1 ? "" : "s"} this week`,
      paragraphs: [
        `${d.count} material change${d.count === 1 ? "" : "s"} across ${d.checked} of ${d.total} entries checked.`,
        ...(d.changes || []).slice(0, 25).map((c) => {
          const move = c.old || c.new ? ` (${c.old || "absent"} → ${c.new || "absent"})` : "";
          return `${c.entryName} · ${c.kind}: ${c.what}${move}${c.editListing ? " — worth editing the listing" : ""}`;
        }),
        (d.changes || []).length > 25 ? `…and ${(d.changes || []).length - 25} more on /admin.` : "",
        "Nothing has been changed. The monitor proposes and never edits a listing.",
      ],
      button: adminButton(d.origin),
    }),
    user: () => null,
...
/*
 * `test` marks a row as having come from the admin test button rather than
 * from something somebody did. It is recorded rather than inferred because the
 * two are otherwise identical on the wire: a test send goes to the admin
 * address, and so does every real admin notification. Without the flag,
 * clearing test rows out of the log later means guessing, and the one rule of
 * the housekeeping panel is that it never guesses what is test data.
 */
async function deliver({ event, cls, to, subject, paragraphs, button, unsubscribe, test = false }) {
  const at = new Date().toISOString();
  try {
    if (!to || (Array.isArray(to) && !to.length)) {
      return { event, cls, to: "", ok: false, error: "no recipient", at, skipped: true, ...(test ? { test } : {}) };
    }
    ...
    return { event, cls, to: Array.isArray(to) ? to.join(", ") : to, ok: true, at, ...(test ? { test } : {}) };
  } catch (e) {
    ...
    return { event, cls, to: Array.isArray(to) ? to.join(", ") : to, ok: false, error, at, ...(test ? { test } : {}) };
  }
}
...
export async function sendEvent(event, data = {}) {
  ...
  try {
    ...
    if (admin) {
      ...
      if (to.length) sends.push(await deliver({ event, cls: "admin", to, ...admin, test: Boolean(data.test) }));
    ...
    if (user && user.to) sends.push(await deliver({ event, cls: "user", ...user, test: Boolean(data.test) }));
