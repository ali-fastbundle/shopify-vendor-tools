 ...
 *  started     optional. YYYY-MM-DD of the first issue. With issueCount this
 *              is how a reader works out whether the run has gaps in it. Omit
 *              both for a publication with no discrete numbered run, such as a
 *              blog with a weekly recap. Omitting is the honest option; there
 *              is no figure to estimate here.
 *  issueCount  optional, and paired with started. A number that goes stale, so
 *              it is only honest next to `updated`.
