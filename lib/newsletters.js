 ...
 *  pricingUrl  optional. Rarely useful for a newsletter, but a paid one
 *              has a page worth watching. See lib/monitor.js.
 *  changelogUrl
 *              optional. For a newsletter this is usually the archive, which
 *              is the page that answers "are they still publishing".
...
export const ALL_NEWSLETTERS = [
  ...
  },
  {
    id: "shopops",
    name: "ShopOps Weekly",
    url: "https://shopifyentrepreneurs.beehiiv.com/",
    publisher: "ShopExperts",
    cadence: "Weekly, Fridays",
    /*
     * Read from the archive: the name, "ShopExperts" in the footer copyright,
     * free with no paywall, and the topic range. Two figures here did not come
     * off the site and should be settled before this is published.
     *
     * The archive states no cadence. "Weekly" is in the publication's own name,
     * so that half stands on its own; "Fridays" does not, and is the editor's.
     *
     * `issueCount` was supplied rather than counted here. The archive paginates
     * and publishes no start date, so `started` stays absent rather than being
     * reverse-engineered from 182 weeks. That is also why the run cannot be
     * checked end to end, which matters because the note calls it gapless.
     */
    issueCount: 182,
    platform: "beehiiv",
    free: true,
    topics: ["platform changes", "AI channels", "app ecosystem", "theme and admin updates", "merchant operations"],
    one: "Weekly digest of what Shopify shipped, written for merchants.",
    note: "182 issues, weekly without a gap, and the most consistent tracker of Shopify platform changes in this section. Covers what actually shipped each week rather than commentary: ChatGPT Ads going live for Shopify, Meta becoming an AI channel, Stocky being retired, variants gaining multiple barcodes, same-day delivery opening to every store. For app vendors the value is the platform-change log: Shopify absorbing a capability is usually the week your roadmap changes, and this catches it without reading the changelog yourself. Also tracks app-market moves, such as Judge.me undercutting Shopify Email on price. Free, no paid tier.",
    watch: "Published by ShopExperts, who also run App Store Research, both listed in this directory. Written for merchants rather than app developers, so app-ecosystem coverage is incidental to the main thread rather than the focus.",
    social: {},
    updated: "2026-09-18",
    draft: true,
  },
