...
export const KEYS = {
  ...
  /* What the monitor has written, and what it replaced, so Undo is exact. */
  changesApplied: "svt:changes:applied",
  /* Which findings have become public feed entries. Independent of applied and
     of dismissed, because one finding can rightly be both published and
     applied: the feed records that it happened, the listing says what is true
     now. */
  changesPublished: "svt:changes:published",
  /* When each scheduled endpoint last fired, and how. */
  cronLast: "svt:cron:last",
};
 ...
 *
 * Writing an empty value and deleting the key are not the same thing. An
 * override's absence means "whatever the file says" and an empty string pins it
 * to nothing (invariant 32), and the same distinction matters wherever a
 * default is read through a missing key. The housekeeping actions in
 * lib/inventory.js want the key gone.
 */
export async function del(key) {
  try {
    if (!usingRedis) { memory.delete(key); return true; }
    await redis(["DEL", key]);
    return true;
  } catch (e) {
    console.error("store.del", key, e.message);
    return false;
  }
}

/*
 * Replace a capped list's contents wholesale.
 *
 * The lists here are append-only by design: LPUSH then LTRIM keeps them bounded
 * without a read and without two writers racing. That is right for writing and
 * useless for removing one row out of the middle, which is what deleting a
 * single mail-log entry is. So this is the deliberate exception, and it is
 * read-modify-write: it is only ever called by a person pressing a button in
 * /admin, never on a request path, so there is nothing to race with.
 *
 * Newest first, matching the order readCapped returns.
 */
export async function writeCapped(key, rows = [], max = 500) {
  const kept = rows.filter(Boolean).slice(0, max).map((e) => JSON.stringify(e));
  try {
    if (!usingRedis) { memory.set(key, kept); return kept.length; }
    /* LPUSH prepends, so pushing the reverse leaves the list newest-first. */
    const cmds = [["DEL", key]];
    if (kept.length) cmds.push(["LPUSH", key, ...kept.slice().reverse()]);
    await pipeline(cmds);
    return kept.length;
  } catch (e) {
    console.error("store.writeCapped", key, e.message);
    return -1;
  }
}

/**
 * A real round trip, for the admin health strip.
 *
 * `read()` swallows its errors on purpose, which is right for a page that
 * should degrade rather than 500, and useless for answering "is the store
 * actually reachable". This one reports the failure instead of hiding it.
 */
export async function ping() {
  if (!usingRedis) {
    return { ok: true, store: "memory", note: "No UPSTASH_* or KV_* env, so this is the in-process fallback. It forgets everything on a cold start." };
  }
  const started = Date.now();
  try {
    await redis(["PING"]);
    return { ok: true, store: "redis", ms: Date.now() - started };
  } catch (e) {
    return { ok: false, store: "redis", ms: Date.now() - started, error: String(e.message).slice(0, 160) };
  }
}
